...
wandb.init(project="nlp_project", name="nlp_project")
...
def train(args):
    ...
    if torch.cuda.is_available():
        ...
        train_dataset = Train_Dataset('new_train_negative1.json', tokenizer_q, tokenizer_c, args.max_length)
        evidence_dataset = Evidence_Dataset('reduced-evidences.json', tokenizer_c, 500)
        ...
        evidence_dataloader = DataLoader(evidence_dataset, batch_size=8, shuffle=False, num_workers=4,
        ...
        # val_f_score = evaluate_dev(question_model, context_model, dev_dataloader, evidence_dataloader, args)
        # print("Evidence Retrieval F-score (F)    =", val_f_score, "\n")
        ...
        torch.save(context_model.state_dict(), 'c_model/context_ckpt.bin')
        torch.save(question_model.state_dict(), 'q_model/question_ckpt.bin')
...
if __name__ == "__main__":
    ...
    # wandb.finish()
# nohup python -u dpr_train.py >train.out 2>&1 &